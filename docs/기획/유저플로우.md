...
버전 2.1 · 2026-09-16 · 현재 로컬 개발본 · [PRD](PRD.md) / [요구사항 정의서](요구사항정의서.md)
...
flowchart TD
    ...
    SCHEDULE -->|건너뛰기| DONE[입력 확인]
    TIMES -->|저장| DONE
    DONE -->|첫 댐 만나기| DAM[내 댐 첫 공개]
    DAM -->|오늘 카드 만나기| FIRST[홈 진입 후 첫 카드 선택 안내]
    DAM -->|같은 세션에서 뒤로| DONE
    RESUME -->|FIRST_DAM| DAM
```
...
| 상황 | 처리와 확인할 점 |
| --- | --- |
| 가입 중 다시 열기 | 저장한 체크포인트·계정 생성 여부를 확인한다. 미완료 계정을 완료한 것으로 보내지 않는다. FIRST_DAM 재개는 완료된 입력을 다시 요구하지 않으며 기본값 요약으로 돌아가는 버튼을 제공하지 않는다. |
| 첫 댐 조회 실패 | 0단계로 추정하지 않는다. 재시도 또는 오늘 카드 선택을 제공한다. 소개 진입은 보상 지급이 아니다. |
...
9/16 첫 댐 소개 추가분의 최신 로컬 소스·Figma 기준·검증은 [첫 댐 문서](../ui-review-2026-09-16/FIRST-DAM.md)에 있다. 위 GitHub 고정 커밋 링크에는 아직 이 로컬 변경이 없다.
...
- 일보 주간면과 쉼 한도의 주간 범위는 월~일이다. 홈·기록의 최근 7일 보고는 이동 기간을 유지한다. 일보에서 아직 오지 않은 날짜는 누를 수 없다.
