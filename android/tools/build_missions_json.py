...
KNOWN_TYPES = {"SELF_CHECK", "SELF_TIMER", "MODEL_ACTIVE_TIME", "MODEL_DISTANCE", "MODEL_STAIR_COUNT"}
...
def main(src: str) -> int:
        ...
        missions.append(
            {
                ...
                "startCondition": r["측정시작조건"],
                "completeRule": r["완료판정"],
                "places": [p.strip() for p in r["행운의위치_후보"].split(",") if p.strip()],
                "numMin": int(r["행운의숫자_최소"]),
                "numMax": int(r["행운의숫자_최대"]),
                "numStep": int(r["행운의숫자_간격"]),
                "unit": r["단위"],
                "timeSlots": [p.strip() for p in r["시간대"].split(",") if p.strip()],
                "fortune": r["오늘의운세_해석"],
                "lineTemplate": r["오늘의한줄_템플릿"],
                "safetyNote": r["수행안내_안전문구"],
                "safetyTag": r["안전태그"],
                "seniorSafe": r["시니어안전"].strip() == "Y",
                "rewardMaterial": mcode,
                "rewardName": mname,
                "rewardHint": mhint,
            }
        )
    ...
    doc = {"version": "1.0", "source": os.path.basename(src), "count": len(missions), "missions": missions}
