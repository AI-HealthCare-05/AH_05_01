...
async def test_v2_invalid_lifestyle_subcomponent_is_excluded_not_clamped():
    result = await _service(health=_health(), habit=_habit(moderate=-1, vigorous=30, strength=8)).get_score_v2(
        _user()
    )
...
async def test_v2_one_valid_lifestyle_subcomponent_is_used_directly():
    result = await _service(habit=_habit(moderate=-1, vigorous=30, strength=2)).get_score_v2(
        _user(complete=False)
    )
