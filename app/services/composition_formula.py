...
def compose(*, health_components, initial, practice_score, practice_version,  # noqa: C901 - 원본 산식 그대로, 복잡도 이유로 리팩토링 금지
            ledger_state, ledger_revision, health_input_revision, as_of):
    ...
    for c in health_components:
        ...
        if (c.get('direction') != 'higher_is_healthier' or
                c.get('valueType') != 'health_direction_peer_percentile'):
    ...
    if any(len({c[name] for c in selected.values()}) != 1
           for name in ('modelVersion', 'referenceVersion')):
    ...
    lifestyle = (None if initial.score is None or practice is None else
                 .2 * initial.score + .8 * practice)
    total = (None if blocked else sum(w * selected[k]['peerPercentile']
             for k, w in zip(HEALTH_KEYS, (.1, .2, .2), strict=True)) + .5 * lifestyle)
    return {
        'schema_version': VERSION,
        'calculation_status': 'incomplete' if blocked else 'calculated',
        'as_of': as_of, 'ledger_state': ledger_state,
        ...
        'health_components': [{k: c[k] for k in (
            'componentKey', 'peerPercentile', 'modelVersion', 'referenceVersion')}
            for c in (selected[key] for key in HEALTH_KEYS)],
        'practice_score': practice, 'lifestyle_score': lifestyle,
        'composite_score': total,
        'display_score': None if total is None else float(
