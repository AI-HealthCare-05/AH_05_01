...
def policy():
    return dict(
        version=VERSION,
        decision="user_selected_2026-09-15",
        deploymentStatus="local_only",
        cumulativeWeight=0.8,
        maintenanceWeight=0.2,
        cumulativeScale=60,
        maintenanceBuildScale=10,
        confirmedRestGraceDays=7,
        dailyMaintenanceRetention=0.98,
        unknownDays="preserve_score_request_update_after_7_days",
        maxDailyUnits=1.5,
        additionalMinutesPerUnit=20,
        standaloneBaseMinutes=5,
        missionBaseUnits=1,
...
def daily_units(events):  # noqa: C901 - 원본 산식 그대로, 복잡도 이유로 리팩토링 금지
    ...
    if not missions:
        ...
        return min(total / 5, 1) + min(max(total - 5, 0) / 20, 0.5)
